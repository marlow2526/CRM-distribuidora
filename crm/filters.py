...
class ProductFilter(django_filters.FilterSet):
    ...
    cost=  django_filters.ModelChoiceFilter(queryset = Product.objects.all(), widget=forms.Select(attrs={'class': 'form-control form-control-sm'}))
    cantidad=  django_filters.ModelChoiceFilter(queryset = Product.objects.all(), widget=forms.Select(attrs={'class': 'form-control form-control-sm'}))    
    class Meta:
        model = Product
        fields = ['name','date_created','cost','cantidad']